...
public abstract class AbstractPicker {
    ...
    protected void initialize(final Context context, int year, int month, int day, ArrayList<Integer> years, ArrayList<Integer> months) {
        ...
        mAlertDialog.getWindow().setGravity(Gravity.CENTER | Gravity.FILL_HORIZONTAL);
        mAlertDialog.getWindow().setBackgroundDrawable(new ColorDrawable(Color.TRANSPARENT));
        mAlertDialog.setCancelable(true);
        mAlertDialog.setCanceledOnTouchOutside(true);
        mAlertDialog.setView(mContentView);
        mAlertDialog.setView(mContentView, 50, 50, 50, 50);
    }
    ...
    public ArrayList<Integer> getDefaultYears() {
        ...
        for (int index = 1930; index <= Calendar.getInstance().get(Calendar.YEAR) + 10; index++) {
            years.add(index);
        }
